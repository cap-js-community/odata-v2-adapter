...
describe("draft-request", () => {
  ...
  it("POST request", async () => {
    ...
    response = await util.callRead(request, `/v2/draft/Header(ID=guid'${id}',IsActiveEntity=false)/Items(ID=guid'${itemId}',IsActiveEntity=false)`);
    expect(response.body).toBeDefined();
    expect(response.body).toMatchObject({
      d: {
        __metadata: {
  ...
  it("POST activate request", async () => {
    ...
    expect(response.statusCode).toEqual(404);
    response = await util.callWrite(request, `/v2/draft/Header_draftPrepare?ID=guid'${id}'&IsActiveEntity=false&SideEffectsQualifier=''`);
    expect(response.statusCode).toEqual(200);
