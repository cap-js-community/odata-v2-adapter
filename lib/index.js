 ...
 * @param {boolean} options.caseInsensitive: Transforms search functions e.g. substringof to case insensitive variant. Default is `false`.
...
function cov2ap(options = {}) {
    ...
    if (headers["sap-messages"]) {
      ...
      if (messages && messages.length > 0) {
        ...
        message.details = messages.map((message) => {
          return convertMessage(message, definition, req, contentID);
        });
  ...
  function convertMessage(message, definition, req, contentID) {
    ...
    message.ContentID = message["@Core.ContentID"] || contentID;
  ...
  function convertResponseError(body, headers, definition, req) {
    ...
    if (body.error) {
      ...
      const contentID = body.error.ContentID;
