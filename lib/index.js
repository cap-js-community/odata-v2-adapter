...
function cov2ap(options = {}) {
  ...
  function lookupDefinition(name, req) {
    ...
    if (definitionName.endsWith("_texts")) {
      const normalizedDefinitionName = definitionName.replace(/_texts$/, ".texts");
      if (req.csn.definitions[normalizedDefinitionName]) {
        return req.csn.definitions[normalizedDefinitionName];
      }
    }
