...
module.exports = (options = {}) => {
  ...
  router.get(`/${path}/*/\\$metadata`, async (req, res) => {
    try {
      ...
      // Trace
      traceResponse(req, "Proxy Response", metadataResponse.status, metadataResponse.statusMessage, headers, metadataBody);
  ...
  function entityUriKey(key, entity, req) {
    ...
    return `${protocol}://${host}${port}${sourcePathPrefix}${sourceServiceEntityPath}(${encodeURIKey(
      key
    )})`;
  }
