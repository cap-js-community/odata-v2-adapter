...
module.exports = (options = undefined) => {
  ...
  function serviceFromRequest(req) {
    ...
    if (!service) {
      ...
      servicePath = servicePath.split("-").map((part) => {
    ...
    if (!req.csn.definitions[service]) {
      throw new Error(`Service definition for request path  '${requestPath}' not found`);
    }
    return {
      name: service,
      path: servicePath
    ...
    const aggregationKey = {
      key: req.context.$apply.key.reduce((result, keyElement) => {
        ...
        if (value !== undefined && value !== null && DataTypeMap[keyElement.type]) {
          value = convertDataTypeToV2Uri(String(value), keyElement.type).replace(/(.*)/, DataTypeMap[keyElement.type].v2);
        }
