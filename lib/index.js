...
module.exports = (options = undefined) => {
  ...
  function serviceFromRequest(req) {
    ...
    let serviceName = Object.keys(cds.services).find(service => {
      ...
      return path === servicePath || path === `/${servicePath}`;
    });
    if (!serviceName) {
    ...
    if (!serviceName || !req.csn.definitions[serviceName]) {
      trace(req, "Service", `Service definition not found for request path '${requestPath}'`);
    }
    return {
      name: serviceName,
      path: servicePath
  ...
  function processFilter(query, fn) {
    if (query["$filter"]) {
      let quote = false;
      let brackets = 0;
      query["$filter"] = query["$filter"]
        ...
        .join("");
      if (fn) {
        query["$filter"] = fn(query["$filter"]);
      }
      query["$filter"] = query["$filter"].replace(/§§([(),])§§/g, "$1");
    }
  }

  function convertFilter(url, req) {
    processFilter(url.query, filter => {
      return (
        filter
          // substringof
          .replace(/substringof§§\(§§(.*?)§§,§§(.*?)§§\)§§/gi, "contains($2,$1)")
          // gettotaloffsetminutes
          .replace(/gettotaloffsetminutes§§\(§§/gi, "totaloffsetminutes(")
      );
    });
  }
