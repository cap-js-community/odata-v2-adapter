...
module.exports = (options = {}) => {
    ...
    try {
      ...
      if (metadataResponse.statusCode === 200) {
        ...
        if (isApplicationJSON(contentType)) {
          body = JSON.stringify(metadataResponse.body);
        } else {
      ...
      respond(req, res, metadataResponse.statusCode, headers, body);
    } catch (err) {
      // Error
      ...
      return await compileService(
        ...
        async () => {
          const response = await request({
            method: "GET",
            baseUrl: target,
            url: `${mtxEndpoint}/metadata/csn/${tenantId}`,
            headers: req.headers
          });
  ...
  async function compileService(locale, tenantId, loadCsn, service) {
    if (!proxyCache[tenantId]) {
      let csnRaw;
      if (cds.model && tenantId === DefaultTenant) {
        csnRaw = cds.model;
      } else {
    ...
    if (context && context.definition && context.definition.elements) {
      ...
      if (contentDispositionFilenameElement && mediaTypeElement) {
        ...
        const response = await request({
          method: "GET",
          baseUrl: target,
          url: [...parts, contentDispositionFilenameElement, "$value"].join("/"),
          headers: req.headers
        });
        const filename = response && response.body;
        if (filename) {
