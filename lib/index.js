...
module.exports = (options = undefined) => {
  ...
  function serviceFromRequest(req) {
    ...
    let serviceName = Object.keys(cds.services).find(service => {
      ...
      if (path) {
        return stripSlashes(path).toLowerCase() === servicePath.toLowerCase();
      }
      return false;
    });
    if (!serviceName) {
    ...
    if (!serviceName || !req.csn.definitions[serviceName]) {
      logWarning(req, "Service", `Service definition not found for request path '${requestPath}'`);
    }
    return {
      name: serviceName,
      path: servicePath
  ...
  async function getTenantMetadataRemote(req, service) {
    ...
    if (tenantId) {
      return await compileService(
        ...
        tenantId,
        async () => {
          const response = await axios.get(target + mtxEndpoint + `/metadata/csn/${tenantId}`, {
            headers: req.headers
          });
  ...
  async function convertProxyRequest(proxyReq, req, res) {
    ...
    if (isMultipart(contentType)) {
      ...
      body = processMultipart(
        ...
        ({ contentType, body, headers, url }) => {
          ...
          delete headers.MaxDataServiceVersion;
          delete headers["if-match"]; // ETAG: cap/issues/3230
          delete headers["If-Match"]; // ETAG: cap/issues/3230
          if (isApplicationJSON(contentType)) {
    ...
    Object.entries(headers).forEach(([name, value]) => {
      if (
        ...
        name === "MaxDataServiceVersion" ||
        name === "if-match" || // ETAG: cap/issues/3230
        name === "If-Match" // ETAG: cap/issues/3230
      ) {
        delete headers[name];
  ...
  async function processStreamResponse(proxyRes, req, res, headers) {
    ...
    if (context && context.definition && context.definition.elements) {
      ...
      if (contentDispositionFilenameElement && mediaTypeElement) {
        ...
        const response = await axios.get(target + [...parts, contentDispositionFilenameElement, "$value"].join("/"), {
          headers: req.headers
        });
        const filename = response && response.data;
        if (filename) {
