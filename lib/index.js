...
module.exports = (options = {}) => {
  ...
  const path = (base ? base + "/" : "") + optionWithFallback("path", "v2");
  ...
  function convertAnalytics(url, req) {
    ...
    if (url.query["$filter"]) {
      for (let name of Object.keys(definition.elements || {})) {
        ...
        const type = elementType(element);
        if (!selects.includes(name) && !(type === "cds.Composition" || type === "cds.Association")) {
          if (
      ...
      if (mediaTypeElement) {
        ...
        if (urlElement) {
          ...
          const contentDispositionFilenameElement =
            findElementByAnnotation(context.definition, "@Core.ContentDisposition.Filename") ||
            findElementByAnnotation(context.definition, "@Common.ContentDisposition.Filename");
