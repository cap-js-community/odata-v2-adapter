...
function cov2ap(options = {}) {
  ...
  function convertResponseList(body, headers, definition, proxyBody, req) {
    if (Array.isArray(proxyBody.value)) {
      if (req.context.aggregationKey) {
        ...
        if (body.d.__delta !== undefined) {
          const options = body.d.__delta.substr(body.d.__delta.indexOf("?"));
          body.d.__delta = rootUri(req) + req.baseUrl + options;
        }
        if (body.d.__delta === undefined && definition["@cov2ap.deltaResponse"] === "timestamp") {
          body.d.__delta = rootUri(req) + req.baseUrl + `?!deltatoken='${new Date().getTime()}'`;
        }
  ...
  function rootUri(req, sourceServicePath) {
    ...
    sourceServicePath = sourceServicePath || `${sourcePath}/${req.servicePath}`;
    ...
    return `${protocol}://${host}${port}${sourcePathPrefix}`;
  }

  function serviceUri(req) {
    return `${rootUri(req)}${sourcePath}/${req.servicePath}`;
  }
  ...
  function entityUriCollection(entity, req) {
    ...
    return `${rootUri(req, sourceServiceEntityPath)}${sourceServiceEntityPath}`;
  }
