 ...
 * @param {boolean} options.isoDateTime Use ISO format for type DateTime. Default is `false`.
...
function cov2ap(options = {}) {
  ...
  function convertDataTypeToV2(value, type, definition) {
    ...
    } else if (!isoDateTime && !definition["@cov2ap.isoDateTime"] && ["cds.DateTime", "cds.Timestamp"].includes(type)) {
      value = `/Date(${new Date(value).getTime()})/`;
    }
    return value;
  }
