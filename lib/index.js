 ...
 * @param [options.standalone] CDS model is retrieved remotely via MTX in multitenant scenario. Default is 'false'.
  ...
  async function getTenantMetadataRemote(req, service) {
    if (req.tenantId) {
      return await compileService(
        ...
        req.tenantId,
        async () => {
          const response = await fetch(`${target}${mtxEndpoint}/metadata/csn/${req.tenantId}`, {
            method: "GET",
            headers: {
              Authorization: req.headers.authorization,
            },
