...
function cov2ap(options = {}) {
  ...
  router.use(
    ...
    async (req, res, next) => {
      ...
      const mediaDataElementName =
        findElementByAnnotation(definition, "@Core.MediaType") ||
        findElementByType(definition, DataTypeOData._Binary) ||
        findElementByType(definition, DataTypeOData.Binary);
  ...
  function findElementByType(definition, type) {
    return (
      definition &&
      definition.elements &&
  ...
  function determineLocale(req) {
    let locale;
    try {
      locale = cds.env.i18n.default_language;
      if (cds.User) {
