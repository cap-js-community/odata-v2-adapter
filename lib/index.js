...
function cov2ap(options = {}) {
    ...
    try {
      ...
      if (body && proxyRes.statusCode < 400) {
        if (isMultipart(contentType)) {
          ...
          if (
            !(req.reqContentIdOrder.length === resContentIdOrder.length) &&
            req.reqContentIdOrder.every((contentId, index) => contentId === resContentIdOrder[index])
          ) {
            throw new Error(`Response change-set order does not match request change-set order`);
          }
        } else {
          // Single
          convertHeaders(body, headers, req);
        ...
        if (urlElement) {
          ...
          if (contentDispositionFilenameElement) {
            ...
            if (response.status === 200) {
              ...
              logError(req, "ContentDisposition", err);
            }
          }
        }
      }
  ...
  function serviceUri(req) {
    ...
    if (req.header("x-forwarded-path") === undefined) {
      ...
      const contextStartRegex = new RegExp(`^${resourceStartPath}[(?].*`);
      const parts = [];
      path.split("/").some((part) => {
        if (resourceStartPath === part || contextStartRegex.test(part)) {
          return true;
        }
        parts.push(part);
        return false;
  ...
  function processMultipart(req, multiPartBody, contentType, urlProcessor, bodyHeadersProcessor, contentIdOrder = []) {
    let maxContentId = new Date().getTime();

    function nextContentID() {
      return String(maxContentId++);
    }
