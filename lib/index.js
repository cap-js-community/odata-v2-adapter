...
function cov2ap(options = {}) {
  ...
  router.use(
    ...
    async (req, res, next) => {
      ...
      const _targetUrl = targetUrl(req);
      const url = parseUrl(_targetUrl, req);
      ...
      const handleMediaEntity = async (contentType, filename, customBody = {}) => {
        try {
          ...
          if (contentDispositionFilenameElementName && filename) {
            body[contentDispositionFilenameElementName] = filename;
          }
          const url = target + _targetUrl;
          const headers = fillLoggingHeaders(req, {
            Authorization: req.headers.authorization,
            "content-type": "application/json",
          });

          // Trace
          traceRequest(req, "ProxyRequest", "POST", url, headers, body);
      ...
      if (isMultipartFormData(req.headers["content-type"])) {
        fileUpload(req, res, async () => {
          await handleMediaEntity(
            ...
            req.body && (req.body["slug"] || req.body["filename"] || req.body["name"]),
            req.body
          );
        });
      } else {
        await handleMediaEntity(
          ...
          req.headers["slug"] || req.headers["filename"] || req.headers["name"],
          req.headers
        );
      }
    },
  ...
  async function getTenantMetadataLocal(req, service) {
    if (req.tenantId && (await cds.mtx.isExtended(req.tenantId))) {
      return await compileService(
        determineLocale(req),
        req.tenantId,
        async () => {
  ...
  async function compileService(locale, tenantId, loadCsn, service) {
    if (!proxyCache[tenantId]) {
      ...
      const csn =
        csnRaw.meta && csnRaw.meta.transformation === "odata"
          ? csnRaw
          : cds.linked(cds.compile.for.odata(csnRaw, { version: "v2" }));
      proxyCache[tenantId] = {
        csnRaw,
        csn,
        edmx: {},
  ...
  function convertUrl(urlPath, req) {
    ...
    const definition = contextFromUrl(url, req);
    enrichRequest(definition, url, req);
    ...
    url.pathname = encodeURIPath(url.pathname);
    return URL.format(url);
  }

  function parseUrl(urlPath, req) {
    const url = URL.parse(urlPath, true);
    url.pathname = decodeURIPath(url.pathname);
    url.basePath = "";
    url.servicePath = "";
  ...
  function enrichRequest(definition, url, req) {
    req.context = {
      url,
    ...
    Object.keys(url.query).forEach((name) => {
      if (name === "$filter") {
        ...
        if (context && context.elements && context.elements[name]) {
          ...
          const type = elementType(element);
          if (DataTypeMap[type]) {
            url.query[name] = convertDataTypeToV4(url.query[name].replace(DataTypeMap[type].v4, "$1"), type);
          }
        }
      ...
      url.contextPath += `(${Object.keys(url.query)
        .reduce((result, name) => {
          if (!name.startsWith("$")) {
            ...
            if (element) {
              const value = encodeURIComponent(url.query[name] || "");
              result.push(`${name}=${quoteParameter(element, value, req)}`);
              delete url.query[name];
            }
          }
  ...
  function convertDataTypesToV4(data, headers, definition, body, req) {
    Object.keys(data || {}).forEach((key) => {
      ...
      if (element) {
        data[key] = convertDataTypeToV4(data[key], elementType(element), headers);
      }
    });
  }
  ...
  async function convertProxyResponse(proxyRes, req, res) {
    try {
      req.context = {};
      let statusCode = (req.overwriteResponse && req.overwriteResponse.statusCode) || proxyRes.statusCode;
      const headers = (req.overwriteResponse && req.overwriteResponse.headers) || proxyRes.headers;
      ...
      // Body
      let body =
        (req.overwriteResponse && req.overwriteResponse.body) || (await parseProxyResponseBody(proxyRes, headers, req));
  ...
  function convertLocation(body, headers, entity, req) {
    const location = entityUri(body, entity, req);
  ...
  function convertResponseList(body, headers, definition, proxyBody, req) {
    if (Array.isArray(proxyBody.value)) {
      if (req.context.aggregationKey) {
        ...
        if (proxyBody["@odata.deltaLink"] !== undefined) {
          body.d.__delta = proxyBody["@odata.deltaLink"];
        } else if (proxyBody["@deltaLink"] !== undefined) {
          body.d.__delta = proxyBody["@deltaLink"];
        }
        if (body.d.__delta !== undefined && body.d.__delta.indexOf("?") >= 0) {
          body.d.__delta = entityUriCollection(definition, req) + body.d.__delta.substr(body.d.__delta.indexOf("?"));
        }
        if (body.d.__delta === undefined && definition["@cov2ap.deltaResponse"] === "timestamp") {
          body.d.__delta = entityUriCollection(definition, req) + `?!deltatoken='${new Date().getTime()}'`;
        }
  ...
  function convertDataTypesToV2(data, headers, definition, body, req) {
    Object.keys(data).forEach((key) => {
      ...
      data[key] = convertDataTypeToV2(data[key], elementType(element));
    });
  }
  ...
  function rootUri(req) {
    ...
    const host =
      ...
      `${req.hostname ? req.hostname.replace("127.0.0.1", "localhost") : "localhost"}:${
        req.socket.address().port || DefaultPort
      }`;
  ...
  function serviceUri(req) {
    ...
    req.context.serviceUri = rootUri(req);
    if (req.header("x-forwarded-path") === undefined) {
      req.context.serviceUri += `${sourcePath}/${req.servicePath}`;
    } else {
      ...
      if (definition) {
        if (definition.kind === "entity") {
          resourceStartPath = definition.name.split(".").pop();
        } else if (definition.kind === "function" || definition.kind === "action") {
          resourceStartPath = (definition.parent || definition).name.split(".").pop();
        }
      }
      const parts = [];
      ...
      if (parts.length > 0) {
        req.context.serviceUri += `/${parts.join("/")}`;
      }
    }
    return req.context.serviceUri;
  }
  ...
  function entityUriCollection(entity, req) {
    return `${serviceUri(req)}/${entity.name.split(".").pop()}`;
  }
  ...
  function elementType(element) {
    ...
    if (element) {
      ...
      if (element["@odata.Type"]) {
        const odataType = element["@odata.Type"].split(".").pop();
  ...
  function processMultipartMixed(
    ...
    parts.forEach((part) => {
      ...
      if (part.startsWith(`--${boundary}`) || (boundaryChangeSet && part.startsWith(`--${boundaryChangeSet}`))) {
        ...
        if (bodyAfterBlank) {
          ...
          if (boundaryChangeSet) {
            ...
            if (!contentTransferEncoding) {
              ...
              // Add content-transfer-encoding to headers of changeset (before url = -3)
        ...
        if (partContentTransferEncoding) {
          const colonIndex = part.indexOf(":");
          if (colonIndex !== -1) {
