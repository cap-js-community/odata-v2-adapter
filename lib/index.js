 ...
 * @param [options.port] Target port, which points to OData V4 backend port. Default is process.env.PORT or '4004'.
 * @param [options.target] Target, which points to OData V4 backend host/port. Default is e.g. 'http://localhost:4004'.
 * @param [options.targetPath] Target path to which is redirected. Default is ''.
 * @param [options.services] Service mapping from url path name to service name. If omitted local CDS defaults apply.
 * @param [options.mtxRemote] CDS model is retrieved remotely via MTX endpoint for multitenant scenario. Default is 'false'.
 * @param [options.mtxEndpoint] Endpoint to retrieve MTX metadata when option 'mtxRemote' is active. Default is '/mtx/v1'.
...
module.exports = (options = {}) => {
  ...
  async function getTenantMetadataRemote(req, service) {
    if (req.tenantId) {
      return await compileService(
        ...
        req.tenantId,
        async () => {
          const mtxBasePath = mtxEndpoint.startsWith("http://") || mtxEndpoint.startsWith("https://") ? mtxEndpoint : `${target}${mtxEndpoint}`;
  ...
  function entityUriKey(key, entity, req) {
    ...
    return `${protocol}://${host}${port}${sourcePathPrefix}${sourceServiceEntityPath}(${key})`;
  }
