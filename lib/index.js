...
module.exports = (options = {}) => {
        ...
        if (context && context.elements && context.elements[name]) {
          ...
          if (DataTypeMap[type]) {
            url.query[name] = convertDataTypeToV4(
              url.query[name].replace(DataTypeMap[type].v4, "$1"),
              type
            );
          }
        }
  ...
  function unquoteParameter(element, value, req) {
    if (
      !element ||
      ["cds.Date", "cds.Time", "cds.DateTime", "cds.Timestamp", "cds.String", "cds.LargeString"].includes(elementType(element))
    ) {
      return value.replace(/^["'](.*)["']$/, "$1");
    }
    return value;
    ...
    const aggregationKey = {
      key: req.context.$apply.key.reduce((result, keyElement) => {
        ...
        if (value !== undefined && value !== null) {
          ...
          if (DataTypeMap[type]) {
            value = convertDataTypeToV2Uri(String(value), type).replace(
              /(.*)/,
              DataTypeMap[type].v2
            );
          }
