...
module.exports = (options = {}) => {
  ...
  router.use(`/${path}/*`, async (req, res, next) => {
    try {
      ...
      logWarning(req, "Authorization", err.stack || err.message);
    }
    next();
  });
  ...
  router.get(`/${path}/*/\\$metadata`, async (req, res) => {
    try {
      ...
      // Error
      logError(req, "MetadataRequest", err.stack || err.message);
  ...
  router.use(
    ...
    async (req, res, next) => {
      try {
        ...
        // Error
        logError(req, "Request", err.stack || err.message);
        res.status(500).send("Internal Server Error");
        return;
      }
      const service = serviceFromRequest(req);
  ...
  function serviceFromRequest(req) {
    ...
    if (!serviceName || !req.csn.definitions[serviceName] || req.csn.definitions[serviceName].kind !== "service") {
      logWarning(req, "Service", `Service definition not found for request path '${servicePath}'`);
    }
    return {
      name: serviceName,
      path: servicePath
        ...
        if (context && context.elements && keyPart) {
          ...
          if (aggregationKey) {
            ...
            try {
              ...
              // Error
              logError(req, "AggregationKey", err.stack || err.message);
            }
          } else {
  ...
  function convertUrlDataTypesForFilterElements(part, entity, navigations = [], depth = 0) {
    if (entity && entity.elements) {
      Object.keys(entity.elements).forEach(name => {
        const element = entity.elements[name];
        if (element.type !== "cds.Composition" && element.type !== "cds.Association") {
          if (DataTypeMap[element.type]) {
            const v4Regex = RegExp(
              `(${[...navigations, element.name].join("/")})(\\s+?(?:eq|ne|gt|ge|lt|le)\\s+?)${
                DataTypeMap[element.type].v4.source
              }`,
          ...
          convertUrlDataTypesForFilterElements(part, element._target, [...navigations, name], depth + 1);
        }
      });
    }
  }
  ...
  async function convertProxyResponse(proxyRes, req, res) {
    try {
      ...
      // Error
      logError(req, "Response", err.stack || err.message);
          ...
          if (mediaResponse) {
            ...
            if (mediaReadLink) {
              try {
                ...
                logError(req, "MediaStream", err.stack || err.message);
  ...
  function convertResponseError(body, headers) {
    ...
    if (body.error) {
      ...
      const detailError = Object.assign({}, body.error);
      delete detailError.innererror;
      delete detailError.details;
      body.error.innererror = body.error.innererror || {};
      body.error.innererror.errordetails = body.error.innererror.errordetails || [];
      body.error.innererror.errordetails.push(detailError);
    ...
    Object.keys(data).forEach(key => {
      if (key.startsWith(AggregationPrefix)) {
        if (!(key.endsWith("@odata.type") || key.endsWith("@type"))) {
          ...
          const aggregationType = (data[`${key}@odata.type`] || data[`${key}@type`] || "#Decimal").replace("#", "cds.");
          let aggregationValue = convertDataTypeToV2(data[key], aggregationType);
        ...
        if (bodyAfterBlank) {
          if (bodyHeadersProcessor) {
            try {
              ...
              // Error
              logError(req, "Batch", err.stack || err.message);
            }
          }
          Object.entries(headers).forEach(([name, value]) => {
  ...
  function logError(req, name, ...messages) {
    const message = messages.filter(message => message !== null && message !== undefined).join("\n");
    req.loggingContext.getLogger(`/${name}`).error(message);
  }

  function logWarning(req, name, ...messages) {
    const message = messages.filter(message => message !== null && message !== undefined).join("\n");
    req.loggingContext.getLogger(`/${name}`).warning(message);
  }
