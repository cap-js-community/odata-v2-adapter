...
function cov2ap(options = {}) {
      ...
      return Object.keys(definition.actions || {}).find((actionName) => {
        if (name.endsWith(`_${actionName}`)) {
          ...
          if (entityDefinition === definition) {
            ...
            req.context.boundDefinition = definition;
            req.context.operation = boundAction;
            const returns =
              (boundAction.returns && boundAction.returns.items && boundAction.returns.items) || boundAction.returns;
            if (returns) {
              req.context.returnDefinition = lookupDefinition(returns.type, req);
            }
            return true;
          }
        }
    ...
    if (!context) {
      ...
      if (context.kind === "function" || context.kind === "action") {
        ...
        if (returns) {
          ...
          req.context.returnDefinition = context;
        }
      }
  ...
  function convertMessage(message, definition, req) {
    ...
    convertMessageTarget(message, req, definition);
    return message;
  }

  function convertMessageTarget(message, req, definition) {
    if (!message.target) {
      return;
    }
    let messageTarget = message.target;
  ...
  function determineLocale(req) {
    if (cds.User) {
      ...
      return require("@sap/cds-runtime/lib/cds-services/adapter/utils/locale")({ req });
    }
  }
