...
module.exports = (options = {}) => {
  ...
  router.get(`/${path}/*/\\$metadata`, async (req, res) => {
    try {
      ...
      // Error
      logError(req, "Error", err.stack || err.message);
  ...
  function convertUrlFilter(filter, context) {
    return buildQuoteParts(filter)
      .map(part => {
        if (!part.quote) {
          convertUrlFilterElements(part, context);
  ...
  function convertUrlFilterElements(part, entity, navigations = [], depth = 0) {
        ...
        if (element.type !== "cds.Composition" && element.type !== "cds.Association") {
          if (DataTypeMap[element.type]) {
            const v4Regex = RegExp(
              `(${[...navigations, element.name].join("/")})(\\s+?(?:eq|ne|gt|ge|lt|le)\\s+?)${DataTypeMap[element.type].v4.source}`,
          ...
          convertUrlFilterElements(part, element._target, [...navigations, name], depth + 1);
        }
      });
    }
  }
