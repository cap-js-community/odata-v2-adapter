...
const DataTypeMap = {
  ...
  "cds.DateTime": { v2: `datetime'$1'`, v4: /datetime'(.+?)'/gi },
  "cds.Timestamp": { v2: `datetime'$1'`, v4: /datetime'(.+?)'/gi },
...
module.exports = (options = undefined) => {
  ...
  if (cds.env.mtx && cds.mtx && cds.mtx.eventEmitter) {
  ...
  async function parseProxyResponseBody(proxyRes, headers, req) {
    ...
    if (req.method === "HEAD") {
      ...
      bodyParser = bodyparser.text({ type: () => true });
