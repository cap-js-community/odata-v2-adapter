 ...
 * @param {boolean} options.continueOnError Indicates to OData V4 backend to continue on error. Default is true.
...
function cov2ap(options = {}) {
  ...
  const continueOnError = optionWithFallback("continueOnError", true);
      ...
      const handleMediaEntity = async (contentType, filename, headers = {}) => {
        try {
          ...
          Object.keys(headers).forEach((name) => {
            ...
            if (element) {
              body[element.name] = convertDataTypeToV4(headers[name], element.type, headers);
            }
          });
  ...
  function convertDataTypeToV4(value, type, headers = {}) {
    ...
    if (["cds.Integer"].includes(type)) {
      value = parseInt(value, 10);
    } else if (["cds.Double"].includes(type)) {
      value = parseFloat(value);
    } else if (["cds.Decimal", "cds.DecimalFloat", "cds.Integer64"].includes(type)) {
  ...
  function convertResponseList(body, headers, definition, proxyBody, req) {
    if (Array.isArray(proxyBody.value)) {
      if (req.context.aggregationKey) {
        ...
        if (!deltaToken && definition["@cov2ap.deltaResponse"] === "timestamp") {
