 ...
 * @param {boolean} options.propagateMessageToDetails: Propagates root error or message to details section. Default is `false`.
 * @param {boolean} options.contentDisposition: Default content disposition for media streams (inline, attachment). Default is `attachment`.
...
function cov2ap(options = {}) {
    ...
    url.contextPath = url.contextPath
      ...
      .map((part) => {
        ...
        if (context && context.elements && keyPart) {
          const aggregationMatch = keyPart.match(/^aggregation'(.*)'$/is);
                ...
                .map((key) => {
                  ...
                  if (name && value) {
                    ...
                    const key = Object.keys(context.keys).find((key) => {
                      return (
                        context.keys[key].type !== "cds.Composition" && context.keys[key].type !== "cds.Association"
                      );
                    });
      ...
      Object.keys(url.query).forEach((name) => {
        if (!name.startsWith("$")) {
          ...
          if (element) {
            ...
            if (Array.isArray(value)) {
              value = value.map((entry) => {
                return unescapeQuote(element, unquoteParameter(element, entry, req), req);
              });
            } else {
              value = unescapeQuote(element, unquoteParameter(element, value, req), req);
              if (element.items && element.items.type) {
                value = [value];
              }
            }
  ...
  function unescapeQuote(element, value, req) {
  ...
  function quoteParameter(element, value, req, quote = "'") {
    if (element && ["cds.String", "cds.LargeString"].includes(elementType(element))) {
      return `${quote}${unquoteParameter(element, value)}${quote}`;
    }
    return value;
  }
  ...
  function convertSearch(url, req) {
    if (url.query.search) {
      url.query["$search"] = `"${url.query.search.replace(/"/g, `\\"`)}"`;
        ...
        if (urlElement) {
          ...
          if (contentDispositionFilenameElement) {
            const contentDispositionTypeValue =
              findElementValueByAnnotation(context.definition, "@Core.ContentDisposition.Type") ||
              findElementValueByAnnotation(context.definition, "@Common.ContentDisposition.Type") ||
              contentDisposition;
  ...
  function convertResponseBody(proxyBody, headers, req) {
    ...
    if (req.context.operation) {
      ...
      } else if (!req.context.definition.kind && req.context.definition.name && req.context.definition.elements.value) {
