...
function cov2ap(options = {}) {
      ...
      return Object.keys(definition.actions || {}).find((actionName) => {
        if (name.endsWith(`_${actionName}`)) {
          ...
          if (entityDefinition === definition) {
            ...
            if (returns) {
              req.lookupContext.returnDefinition = lookupDefinition(returns.type, req);
            }
            return true;
          }
        }
  ...
  function convertResponseBody(proxyBody, headers, req) {
    ...
    if (req.context.operation) {
      if (req.context.definition.kind === "type" && !Array.isArray(body.d.results)) {
        const localOperationName = req.context.operation.name.split(".").pop();
        body.d = {
