...
module.exports = (options = undefined) => {
  ...
  async function compileService(req, tenantId, loadCsn, service) {
    const language = determineLocale(req);
    ...
    if (service) {
      ...
      if (!(proxyCache[tenantId].edmx[service] && proxyCache[tenantId].edmx[service][language])) {
        const csnLocalized = cds.localize(proxyCache[tenantId].csnRaw, language);
        proxyCache[tenantId].edmx[service][language] = await cds.compile.to.edmx(csnLocalized, {
          service,
          version: "v2"
        });
      }
    ...
    return {
      ...
      edmx: service && proxyCache[tenantId].edmx[service][language]
    };
  }
  ...
  async function convertProxyRequest(proxyReq, req, res) {
    // Trace
    traceRequest(req, "Request", req.method, req.originalUrl, req.body);
