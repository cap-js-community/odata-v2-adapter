...
function cov2ap(options = {}) {
  ...
  async function prepareMetadata(locale, tenantId, loadCsn, loadEdmx, service) {
    if (!(proxyCache[tenantId] && proxyCache[tenantId].csn)) {
      ...
      // TODO: Remove cds.compile.for.odata
      const csn =
              ...
              `${part}(${keys
                .map((key) => {
                  ...
                  if (name && value) {
                    ...
                    if (!type) {
                      type = context.elements[name] && elementType(context.elements[name], req);
                    }
                    ...
                    type = key && context.elements[key] && elementType(context.elements[key], req);
  ...
  function convertValue(url, req) {
    if (url.contextPath.endsWith("/$value")) {
      ...
      const mediaDataElementName =
        req.context && req.context.definition && findElementByAnnotation(req.context.definitionElements, "@Core.MediaType");
  ...
  function convertLocation(body, headers, definition, elements, req) {
    const location = entityUri(body, definition, elements, req);
  ...
  function definitionElements(definition) {
    // TODO: Flatten managed foreign keys. Lookup managed associations, compositions and add primary key as foreign keys to elements
    return (definition && definition.elements) || {};
  }
  ...
  function findElementByType(elements, type, req) {
    return (
      Object.keys(elements)
        .filter((key) => {
          return !(
            elements[key].type === "cds.Composition" && elements[key].type === "cds.Association"
          );
        })
        .find((key) => {
          const element = elements[key];
