 ...
 * @param [options.disableNetworkLog] Disable networking logging
 * @returns {Router}
 */
module.exports = (options = {}) => {
  ...
  const disableNetworkLog = optionWithFallback("disableNetworkLog", false);
  ...
  router.use(`/${path}/*`, logging.middleware({ appContext: appContext, logNetwork: !disableNetworkLog && !process.env.DISABLE_NETWORK_LOG }));
          ...
          if (element.type !== "cds.Composition" && element.type !== "cds.Association") {
            if (DataTypeMap[element.type]) {
              const v4Regex = RegExp(
                `(${namePath})(\\)?\\s+?(?:eq|ne|gt|ge|lt|le)\\s+?)${
